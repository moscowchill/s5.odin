#!/bin/bash
# Build script for Stealth SOCKS5 Proxy Server
...
if $ODIN build $PROJECT -file -target:windows_amd64 -o:speed -no-bounds-check -out:s5proxy.exe 2>/dev/null; then
    ...
    echo "⚠ Windows cross-compilation not supported on this platform"
